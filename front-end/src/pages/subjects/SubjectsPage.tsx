                        ...
                        <List.Item className="plan-subject-row" key={item.id}>
                            <Input
                                ...
                                onChange={event => {
                                    setSubjects(prevState => {
                                        let newSubjects = [...prevState];
                                        newSubjects.find((x)=>{return x.id === item.id})!.name = event.target.value;
                                        return newSubjects;
                                    })
                                }}
                            />
                            <InputNumber
                                ...
                                onChange={event => {
                                    setSubjects(prevState => {
                                        let newSubjects = [...prevState];
                                        newSubjects.find((x)=>{return x.id === item.id})!.ects = event!;
                                        return newSubjects;
                                    })
                                }}
                            />
                            <Button
                                ...
                                onClick={()=>{
                                    setSubjects(prevState => {
                                        let newSubjects = [...(prevState.filter((x)=>{return x.id !== item.id}))];
                                        return newSubjects;
                                    })
                                }}
                            >
