    ...
    <div id="plan-page-container">
        <div id="plan-list" className="block">
            ...
            <List
                dataSource={plans}
                renderItem={item => (
                    <List.Item onClick={()=>{loadPlan(item.id.toString())}} style={{display:"block", margin:"0"}} key={item.id}>
                        ...
                        <Input
                            ...
                            onChange={event => {
                                setCurrentPlan(prevState => {
                                    let newPlan : Plan = {...prevState!, name: event.target.value, planSubjects: [...(prevState!.planSubjects!)]};
                            ...
                            <List.Item className="plan-subject-row" key={item.id}>
                               ...
                               <div className="plan-subject-destination">
                                    <Input
                                        ...
                                        onChange={event => {
                                            setCurrentPlan(prevState => {
                                                let newPlan : Plan = {...prevState!, planSubjects: [...(prevState!.planSubjects!)]};
                                                newPlan.planSubjects!.find((x)=>{return x.id === item.id})!.name = event.target.value;
                                    ...
                                    <InputNumber
                                        ...
                                        onChange={event => {
                                            setCurrentPlan(prevState => {
                                                let newPlan : Plan = {...prevState!, planSubjects: [...(prevState!.planSubjects!)]};
                                                newPlan.planSubjects!.find((x)=>{return x.id === item.id})!.ects = event!;
                                                return newPlan;
                                            });
                                        }}
                                    />
